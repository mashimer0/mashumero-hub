...
local function spawnToolPencil()
    ...
    task.spawn(function()
        pcall(function() spawnRemote:InvokeServer("ToolPencil", spawnPos, Vector3.zero) end)
    end)
    local start = tick()
...
local function setNetworkOwnerPencil(part)
    ...
    pcall(function()
        for i = 1, 5 do setNet:FireServer(part, part.CFrame) end
    end)
end
...
local function spawnBanana()
    ...
    task.spawn(function()
        pcall(function() spawnRemote:InvokeServer("FoodBanana", spawnPos, Vector3.zero) end)
    end)
    local start = tick()
    repeat task.wait(0.05) until spawned or (tick() - start) > 3
    if conn then pcall(function() conn:Disconnect() end) end
...
playerDropdown = RagdollGroup:AddDropdown("RagdollTargetSelect", {
    Text = "Select Player",
    Default = "",
    Values = GetPlayerList(),
    Callback = function(v)
        local name = GetPlayerNameFromDisplay(v)
        if name then selectedPlayer = name else selectedPlayer = nil end
    end
})

local function UpdatePlayerList()
    if playerDropdown then playerDropdown:SetValues(GetPlayerList()) end
end

Players.PlayerAdded:Connect(function() task.wait(0.5) UpdatePlayerList() end)
Players.PlayerRemoving:Connect(function() task.wait(0.2) UpdatePlayerList() end)
task.spawn(function() task.wait(1) UpdatePlayerList() end)
...
RagdollGroup:AddToggle("RagdollKill", {
    ...
    Callback = function(v)
        if v then
            ...
            if ragdollMonitorTask then task.cancel(ragdollMonitorTask) ragdollMonitorTask = nil end
            destroyAllPencils()
        end
    end
})
